...
def Holzapfel_analytical_uniaxial(lbda, matparams={}):
	...
	def func(lbda2):
		...
		return a*math.exp(b*(I1(lbda1, lbda2) - 3))*lbda2**2 + \
		       2*a_s*max((I4(lbda2) - 1),0)*math.exp(b_s*(max(I4(lbda2) - 1,0))**2)*lbda2**2 - \
	...
	sigma11 = a*math.exp(b*(I1(lbda1, lbda2) - 3))*lbda1**2 + 2*a_f*(max(I4(lbda1) - 1,0))*math.exp(b_f*(max(I4(lbda1) - 1,0))**2.0)*lbda1**2 - p
	PK11 = sigma11/lbda1
