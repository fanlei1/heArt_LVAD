...
############
	...
	default_params = {"EDP":12, "maxit":20, "restol":1e-3, "drestol":1e-4, "volinc":1, "LVangle":[60,-60]}
...
################        
        Tmax1 = self.Tmax1
        Tmax1.value = 1.0  #Qlad_total*1.41-0.1985  
        #####
        Tmax2 = self.Tmax2
        Tmax2.value = 1.0
        Tmax3 = self.Tmax3
...
################
		...
		for regionid in self.SimDet["active_region"]:
			if(region_cnt == 0):
                                F4 = Tmax1*inner(Fmat*Sactive, grad(v_me))*(dx_me(int(regionid))) 
        			#F4 = inner(Fmat*Sactive, grad(v_me))*(dx_me(int(regionid)))
				print "Assigning active stress to ", regionid
                        elif(region_cnt == 0):
                                F4 += Tmax2*inner(Fmat*Sactive, grad(v_me))*(dx_me(int(regionid)))        
			else:
                                F4 += Tmax3*inner(Fmat*Sactive, grad(v_me))*(dx_me(int(regionid))) 
        			#F4 += inner(Fmat*Sactive, grad(v_me))*(dx_me(int(regionid)))
				print "Assigning active stress to ", regionid
			...
			region_cnt += 1
        		...
        		#F4 = inner(Fmat*Sactive, grad(v_me))*dx_me
                        F4 = Tmax1*inner(Fmat*Sactive, grad(v_me))*dx_me(0) + Tmax2*inner(Fmat*Sactive, grad(v_me))*dx_me(1) + Tmax3*inner(Fmat*Sactive, grad(v_me))*dx_me(2)
        Ftotal = F1 + F2 + F4  
